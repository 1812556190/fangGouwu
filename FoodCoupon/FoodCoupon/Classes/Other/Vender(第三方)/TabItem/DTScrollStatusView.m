...
-(void)setStatusViewWithTitle:(NSArray *)titleArr
{
    ...
    if (curNormalTabColor && curSelectTabColor) {
        [self.statusView setUpStatusButtonWithTitlt:titleArr NormalColor:curNormalTabColor SelectedColor:curSelectTabColor LineColor:curSelectTabColor];
    }
    else
    {
    ...
    for ( int i = 0; i < titleArr.count; i++) {
        UITableView *table = [[UITableView alloc]initWithFrame:CGRectMake(kScreenWidth*i, 0, kScreenWidth, mainScrollH) style:UITableViewStyleGrouped];
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    if (_scrollStatusDelegate) {
        if ([_scrollStatusDelegate respondsToSelector:@selector(tableView:heightForFooterInSection:)]) {
            return [_scrollStatusDelegate tableView:tableView heightForFooterInSection:section];
        }
    }
    return CGFLOAT_MIN;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (_scrollStatusDelegate) {
        if ([_scrollStatusDelegate respondsToSelector:@selector(tableView:viewForFooterInSection:)]) {
            return  [_scrollStatusDelegate tableView:tableView viewForFooterInSection:section];
        }
    }
    return nil;
}
